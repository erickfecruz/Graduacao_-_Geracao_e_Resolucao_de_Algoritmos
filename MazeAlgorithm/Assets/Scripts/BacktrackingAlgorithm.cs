...
public class BacktrackingAlgorithm : MonoBehaviour {
	[System.Serializable]
	public class Celula {
	...
	private int vizAtual = 0;
	private int voltando = 0;
	int posCelAtiva = 0;
	private int paredeParaQuebrar = 0;
	...
	//Função para gerar o labirinto por meio do Algoritmo de Backtracking
	IEnumerator CreateMaze(){
		celAtiva = new List<int> ();
		celAtiva.Clear();

		celAtual = Random.Range (0, totalCel);
		celulas [celAtual].visitado = true;
		celAtiva.Add (celAtual);
		posCelAtiva = 1;

		while (celAtiva.Count != 0) {
			Debug.Log (celAtiva.Count);
			GiveMeNeighbour ();
			if (celulas[vizAtual].visitado == false) {
				BreakWall ();
				celulas [vizAtual].visitado = true;
				celAtiva.Add (vizAtual);
				celAtual = vizAtual;
			}
	...
	void BreakWall(){
		switch (paredeParaQuebrar) {
		case 1 : Destroy (celulas [celAtual].norte); break;
		case 2 : Destroy (celulas [celAtual].sul); break;
		case 3 : Destroy (celulas [celAtual].leste); break;
		case 4 : Destroy (celulas [celAtual].oeste); break;
		}
	}
	...
	void GiveMeNeighbour(){
		...
		int check = 0;
		celAtual = celAtiva [celAtiva.Count - 1];
		check = ((celAtual + 1) / xSize);
		check -= 1;
		check *= xSize;
		check += xSize;
		...
		//Leste
		if (celAtual + 1 < totalCel && (celAtual + 1) != check) {
			if (celulas[celAtual + 1].visitado == false) {
				neighbours [lenght] = celAtual + 1;
		...
		//Oeste
		if (celAtual - 1 >= 0 && celAtual != check) {
			if (celulas[celAtual - 1].visitado == false) {
				neighbours [lenght] = celAtual - 1;
		...
		//Norte
		if (celAtual + xSize < totalCel) {
			if (celulas[celAtual + xSize].visitado == false) {
				neighbours [lenght] = celAtual + xSize;
		...
		if (lenght != 0) {
			int escolhida = Random.Range (0, lenght);
			vizAtual = neighbours [escolhida];
			paredeParaQuebrar = connectingWall [escolhida];
		} else {
			celAtiva.RemoveAt (celAtiva.Count - 1);
		}
		menu2.gameObject.SetActive (true);
	}
